...
-- Tối ưu hóa tên thư mục để tránh bị Sandbox của Executor chặn
local folderName = "PuriumHub"
pcall(function() if not isfolder(folderName) then makefolder(folderName) end end)

local WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"))()
local Window = WindUI:CreateWindow({
    Title = "Purium Hub [By @hlck49] | SBSD |", Icon = "door-open", Author = "Version : 0.0.4", Folder = folderName,
    ...
    pcall(function()
        Window:Dialog({
            ...
            Buttons = {
                ...
                { Title = "Cool Boy", Callback = function() end, Variant = "Primary" }
            }
        })
    end)
end)
...
local function setAutoLoad(name)
    pcall(function() if not isfolder(folderName) then makefolder(folderName) end writefile(autoLoadPath, name) end)
end
...
InfoSection:Paragraph({ Title = "Welcome To Purium Hub!", Desc = "Note : Thank You For Using My Script :D !!", Image = "alert-triangle", ImageSize = 5 })
InfoSection:Paragraph({ Title = "Owner | Developers", Desc = "@hlck49 [Owner]\nHieu&Thanh [Developers]" })

InfoTab:Space()
local UpdateSection = InfoTab:Section({ Title = "Changelogs & Updates", Icon = "history", Opened = true, Box = true })
...
AutoFarmSection:Toggle({
    ...
    Callback = function(_Value)
        ...
        if _Value then
            _FarmStep = 1; _HasWaited = false; _G.IsNoclipping = false
            task.spawn(function()
                local _call29 = workspace:FindFirstChild('SafePlatform') or Instance.new('Part')
                if not _call29.Parent then
                    _call29.Size = Vector3.new(50, 5, 50); _call29.Anchored = true; _call29.Position = Vector3.new(36.8919563, 400, -4.66330338); _call29.Transparency = 1; _call29.Name = 'SafePlatform'; _call29.Parent = workspace
                end
                ...
                while _G.AutoFarmActive do
                    pcall(function()
                        ...
                        if game.PlaceId == 14852797539 then
                            _G.IsNoclipping = false; hrp.CFrame = CFrame.new(-82, 20, 157); task.wait(1)
                            ...
                            if closestElevator then hrp.CFrame = closestElevator.CFrame; task.wait(0.5); local prompt = closestElevator:FindFirstChildOfClass("ProximityPrompt"); if prompt then fireproximityprompt(prompt) end end
                            task.wait(2) 
                        else
                            local playerGui = _LocalPlayer22:FindFirstChild('PlayerGui'); local charClass = playerGui and playerGui:FindFirstChild('CharClass')
                            local hasUI = charClass and charClass:FindFirstChild('Scroll') and charClass.Scroll.Visible == true

                            if hrp.Position.Y > 200 and hrp.Position.Y < 350 and _FarmStep >= 3 then _FarmStep = 1; _HasWaited = false; _G.IsNoclipping = false; task.wait(1) end
                            ...
                            if _FarmStep == 1 then
                                if hasUI then _FarmStep = 2 else
                                    ...
                                    if sandBtn then
                                        ...
                                        if prompt then prompt.HoldDuration = 0; fireproximityprompt(prompt); _call21:SendKeyEvent(true, Enum.KeyCode.E, false, game); task.wait(0.2); _call21:SendKeyEvent(false, Enum.KeyCode.E, false, game) end
                                    end
                                end
                            elseif _FarmStep == 2 then
                                if hasUI then
                                    local scroll = charClass.Scroll; local saw = scroll:FindFirstChild('Frame') and scroll.Frame:FindFirstChild('Saw')
                                    if saw then
                                        local pObj = saw.Parent; while pObj and pObj:IsA("GuiObject") do pcall(function() pObj.ClipsDescendants = false end); pObj = pObj.Parent end
                                        pcall(function() saw.AnchorPoint = Vector2.new(0.5, 0.5); saw.Position = UDim2.new(0.5, 0, 0.5, 0); saw.Size = UDim2.new(100, 0, 100, 0); saw.ZIndex = 999999999; saw.Active = true end)
                                        task.wait(0.2); UniversalClick(saw)
                                        pcall(function() local cam = workspace.CurrentCamera; local mX, mY = cam.ViewportSize.X/2, cam.ViewportSize.Y/2; _call21:SendMouseButtonEvent(mX, mY, 0, true, game, 1); task.wait(0.05); _call21:SendMouseButtonEvent(mX, mY, 0, false, game, 1) end)
                                        task.wait(1)
                                    end
                            ...
                            elseif _FarmStep == 4 then
                                ...
                                if enemy and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                                    if not _HasWaited then _G.IsNoclipping = false; if _call29 then hrp.CFrame = _call29.CFrame * CFrame.new(0, 3, 0) end; task.wait(_farmSpeed); _HasWaited = true end
                                    if _HasWaited then _G.IsNoclipping = true; hrp.CFrame = enemy.HumanoidRootPart.CFrame; task.spawn(function() _call21:SendKeyEvent(true, Enum.KeyCode.F, false, game); task.wait(0.05); _call21:SendKeyEvent(false, Enum.KeyCode.F, false, game) end) end
                                else
                                    _HasWaited = false; _G.IsNoclipping = false; if _call29 then hrp.CFrame = _call29.CFrame * CFrame.new(0, 3, 0) end
                                end
                            end
                        end
                    end)
...
AutoFarmSection:Toggle({
    Title = "Show Stats", Desc = "Show Points earned & current", Flag = "ShowStatsToggle", Value = false,
    Callback = function(Value)
        if Value then
            local success = pcall(function() 
                local _call150 = Instance.new('ScreenGui')
                _call150.Name = 'StatsGUI'
                local coreTarget = (gethui and gethui()) or game:GetService("CoreGui")
                if not pcall(function() _call150.Parent = coreTarget end) then _call150.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui") end

                local _call152 = Instance.new('Frame', _call150)
                _call152.Name = 'Quorum'; _call152.BackgroundColor3 = Color3.fromRGB(0, 0, 0); _call152.BorderColor3 = Color3.fromRGB(255, 255, 255); _call152.BorderSizePixel = 0; _call152.Position = UDim2.new(0.7, 0, 0.22, 0); _call152.Size = UDim2.new(0.29, 0, 0.46, 0)
                local _call154 = Instance.new('UICorner', _call152); _call154.CornerRadius = UDim.new(0, 20)
                local _call156 = Instance.new('TextLabel', _call152); _call156.Name = 'Points'; _call156.Size = UDim2.new(0.47, 0, 0.15, 0); _call156.Position = UDim2.new(0, 0, 0.04, 0); _call156.BackgroundTransparency = 1; _call156.TextColor3 = Color3.fromRGB(255, 255, 255); _call156.Font = Enum.Font.PermanentMarker; _call156.TextScaled = true; _call156.Text = 'POINTS:'
                local _call158 = Instance.new('TextLabel', _call152); _call158.Name = 'Counter'; _call158.Size = UDim2.new(0.45, 0, 0.15, 0); _call158.Position = UDim2.new(0.5, 0, 0.04, 0); _call158.BackgroundTransparency = 1; _call158.TextColor3 = Color3.fromRGB(255, 255, 255); _call158.Font = Enum.Font.PermanentMarker; _call158.TextScaled = true; _call158.Text = '0'
                local _call160 = Instance.new('TextLabel', _call152); _call160.Name = 'PointsEarned'; _call160.Size = UDim2.new(0.47, 0, 0.15, 0); _call160.Position = UDim2.new(0, 0, 0.23, 0); _call160.BackgroundTransparency = 1; _call160.TextColor3 = Color3.fromRGB(255, 255, 255); _call160.Font = Enum.Font.PermanentMarker; _call160.TextScaled = true; _call160.Text = 'EARNED:'
                local _call162 = Instance.new('TextLabel', _call152); _call162.Name = 'CounterEarned'; _call162.Size = UDim2.new(0.45, 0, 0.15, 0); _call162.Position = UDim2.new(0.5, 0, 0.23, 0); _call162.BackgroundTransparency = 1; _call162.TextColor3 = Color3.fromRGB(255, 255, 255); _call162.Font = Enum.Font.PermanentMarker; _call162.TextScaled = true; _call162.Text = '0'
                local _call164 = Instance.new('TextLabel', _call152); _call164.Name = 'TimeElapsed'; _call164.Size = UDim2.new(0.77, 0, 0.3, 0); _call164.Position = UDim2.new(0.11, 0, 0.38, 0); _call164.BackgroundTransparency = 1; _call164.TextColor3 = Color3.fromRGB(255, 255, 255); _call164.Font = Enum.Font.PermanentMarker; _call164.TextScaled = true; _call164.Text = 'TIME ELAPSED:'
                local _call166 = Instance.new('TextLabel', _call152); _call166.Name = 'TimeCount'; _call166.Size = UDim2.new(0.74, 0, 0.26, 0); _call166.Position = UDim2.new(0.11, 0, 0.59, 0); _call166.BackgroundTransparency = 1; _call166.TextColor3 = Color3.fromRGB(255, 255, 255); _call166.Font = Enum.Font.PermanentMarker; _call166.TextScaled = true; _call166.Text = '00:00:00'
                
                local _points = game:GetService("Players").LocalPlayer:WaitForChild('leaderstats', 5)
                _points = _points and _points:WaitForChild('Points', 5)
            ...
            pcall(function()
                local coreTarget = (gethui and gethui()) or game:GetService("CoreGui")
                if coreTarget:FindFirstChild("StatsGUI") then coreTarget.StatsGUI:Destroy() end
                if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("StatsGUI") then game:GetService("Players").LocalPlayer.PlayerGui.StatsGUI:Destroy() end
            end)
...
-- [FIX TRIỆT ĐỂ LỖI ESCAPE DIRECTORY MÀ BẠN CHỤP TRONG ẢNH]
-- Bỏ quét thư mục bằng hàm AllConfigs() của Wind UI để tránh Sandbox chặn lại.
...
ConfigSection:Button({ 
    ...
    Callback = function() 
        ...
        if configFile:Save() then 
            -- Tự động thêm tên cấu hình vào bộ nhớ ảo thay vì quét file
            local exists = false
            for _, v in pairs(savedConfigs) do if v == configName then exists = true break end end
            if not exists then 
                table.insert(savedConfigs, configName)
                ConfigDropdown:Refresh(savedConfigs) 
            end
