        ...
        if _G.DesyncGodMode and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if tick() - _G.LastAttackTime > 0.2 then
                if hrp.Position.Y < 20000 then
                    local vel = hrp.AssemblyLinearVelocity; local rot = hrp.AssemblyAngularVelocity
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if _G.HitboxStatus then
                    hrp.Size = Vector3.new(_G.HitboxSize, _G.HitboxSize, _G.HitboxSize); hrp.CanCollide = false
                    ...
                    hrp.Size = Vector3.new(2, 2, 1); hrp.Transparency = 1; if hrp:FindFirstChild("HitboxSelection") then hrp.HitboxSelection:Destroy() end
                end
            end
        end
    end)
...
MoveSec:Toggle({ Title = "Enable Noclip", Desc = "Walk through walls and obstacles", Value = false, Callback = function(v)
    if v then 
        noclipLoop = RunService.Stepped:Connect(function() pcall(function() if LocalPlayer.Character then for _, p in pairs(LocalPlayer.Character:GetDescendants()) do if p:IsA("BasePart") and p.CanCollide then p.CanCollide = false end end end end) end) 
...
RunService.RenderStepped:Connect(function()
    if (_G.AimbotMode == "Camera" or _G.AimbotMode == "Camera & Character") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local target = getNearestTarget()
        if target and target:FindFirstChild("HumanoidRootPart") then
            local tPos = target.HumanoidRootPart.Position; local goalCFrame = CFrame.lookAt(Camera.CFrame.Position, tPos)
...
VisSec:Toggle({ Title = "Show Invisible Players", Desc = "Force hidden players to appear normally", Value = false, Callback = function(v)
    if v then
        antiInvisLoop = RunService.RenderStepped:Connect(function()
            pcall(function()
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character then for _, part in ipairs(p.Character:GetDescendants()) do if (part:IsA("BasePart") or part:IsA("Decal")) then if part.Name ~= "HumanoidRootPart" and not string.find(string.lower(part.Name), "hitbox") then if part.Transparency > 0 then if originalTrans[part] == nil then originalTrans[part] = part.Transparency end; part.Transparency = 0 end end end end end
                end
            end)
        end)
    else
            ...
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer then
                    ...
                    if char and hrp and hum and head and hum.Health > 0 then
                        ...
                        if onScreen then
                            local headPos = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.5, 0)); local legPos = Camera:WorldToViewportPoint(hrp.Position - Vector3.new(0, 3, 0)); local boxHeight = math.abs(headPos.Y - legPos.Y); local boxWidth = boxHeight * 0.6; local boxPos = Vector2.new(rootPos.X - boxWidth / 2, headPos.Y)
