...
for _, theme in ipairs(themesData) do WindUI:AddTheme(theme) end
...
local function getFakeCamPart()
    ...
    if not cp then cp = Instance.new("Part"); cp.Name = "PuriumCamPart"; cp.Transparency = 1; cp.CanCollide = false; cp.Anchored = true; cp.Massless = true; cp.Size = v3_new(1, 1, 1); cp.Parent = Workspace end
    return cp
end
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if tick_now() - _G.LastAttackTime > 0.2 then
                if hrp.Position.Y < 20000 then
                    ...
                    hrp.AssemblyLinearVelocity = vel; hrp.AssemblyAngularVelocity = rot
                end
            else
                if hrp.Position.Y > 20000 then
                    local vel, rot = hrp.AssemblyLinearVelocity, hrp.AssemblyAngularVelocity
...
RunService.RenderStepped:Connect(function()
    if _G.DesyncGodMode then
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            local cp = getFakeCamPart(); cp.CFrame = hrp.CFrame; Camera.CameraSubject = cp
        end
    end
end)
...
MoveSec:Divider()
_G.WsEnabled = false; _G.WsValue = 25
...
MoveSec:Divider()
_G.JpEnabled = false; _G.JpValue = 100
...
RunService.Heartbeat:Connect(function()
    if _G.FlingActive and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if _G.FlingMode == "Spin Fling" then
            ...
            if not bav then bav = Instance.new("BodyAngularVelocity"); bav.Name = "PuriumFlingBAV"; bav.MaxTorque = v3_new(0, math.huge, 0); bav.P = math.huge; bav.Parent = hrp end
...
_G.AimbotMode = "None"; _G.AimbotSmoothness = 1
...
RunService.Heartbeat:Connect(function()
    if (_G.AimbotMode == "Character" or _G.AimbotMode == "Camera & Character") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        local target = getNearestTarget()
        if target and target:FindFirstChild("HumanoidRootPart") then
            local myHrp = LocalPlayer.Character.HumanoidRootPart; local tPos = target.HumanoidRootPart.Position
...
VisSec:Toggle({ Title = "Show Invisible Players", Desc = "Force hidden players to appear normally", Value = false, Callback = function(v)
    if v then
        antiInvisLoop = RunService.RenderStepped:Connect(function()
            pcall(function()
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character then for _, part in ipairs(p.Character:GetDescendants()) do if (part:IsA("BasePart") or part:IsA("Decal")) then if part.Name ~= "HumanoidRootPart" and not str_find(str_lower(part.Name), "hitbox") then if part.Transparency > 0 then if originalTrans[part] == nil then originalTrans[part] = part.Transparency end; part.Transparency = 0 end end end end end
                end
            end)
        end)
    else
        if antiInvisLoop then antiInvisLoop:Disconnect(); antiInvisLoop = nil end
        for part, trans in pairs(originalTrans) do pcall(function() if part and part.Parent then part.Transparency = trans end end) end; originalTrans = {}
    end
end})

VisSec:Divider()
_G.ESP_Box = false; _G.ESP_Name = false; _G.ESP_Chams = false
        ...
        if _G.ESP_Box or _G.ESP_Name or _G.ESP_Chams then
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer then
                    activePlayers[player] = true; local char = player.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart"); local hum = char and char:FindFirstChild("Humanoid"); local head = char and char:FindFirstChild("Head")
                    if char and hrp and hum and head and hum.Health > 0 then
                        if not espElements[player] then
                            ...
                            d.Name.Size = 16; d.Name.Center = true; d.Name.Outline = true; d.Name.Font = 2; d.Box.Thickness = 1; d.Box.Filled = false; d.Highlight.FillTransparency = 0.5; d.Highlight.OutlineTransparency = 0; d.Highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; d.Highlight.Parent = char; espElements[player] = d
                        end
                        local d = espElements[player]; local rootPos, onScreen = Camera:WorldToViewportPoint(hrp.Position)
                        if onScreen then
                            local headPos = Camera:WorldToViewportPoint(head.Position + v3_new(0, 0.5, 0)); local legPos = Camera:WorldToViewportPoint(hrp.Position - v3_new(0, 3, 0)); local boxHeight = math_abs(headPos.Y - legPos.Y); local boxWidth = boxHeight * 0.6; local boxPos = Vector2.new(rootPos.X - boxWidth / 2, headPos.Y)
                            local dist = (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")) and (LocalPlayer.Character.HumanoidRootPart.Position - hrp.Position).Magnitude or 0; local teamColor = player.TeamColor and player.TeamColor.Color or Color3.fromRGB(255, 50, 50)
                            d.Box.Size = Vector2.new(boxWidth, boxHeight); d.Box.Position = boxPos; d.Box.Color = teamColor; d.Box.Visible = _G.ESP_Box; d.Name.Color = teamColor; d.Name.Text = player.Name .. " ["..math_floor(dist).."m]"; d.Name.Position = Vector2.new(rootPos.X, headPos.Y - 20); d.Name.Visible = _G.ESP_Name; d.Highlight.FillColor = teamColor; d.Highlight.Parent = char; d.Highlight.Enabled = _G.ESP_Chams
                        else d.Name.Visible = false; d.Box.Visible = false; d.Highlight.Enabled = false end
...
local function refreshPlayers() playerNames = {}; for _, p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer then t_insert(playerNames, p.Name) end end; PlayerDropdown:Refresh(playerNames) end
...
local validThemes = WindUI:GetThemes(); local uiThemes = {}
for themeName, _ in pairs(validThemes) do t_insert(uiThemes, themeName) end; table.sort(uiThemes)
...
local function getAutoLoad() local success, result = pcall(function() if isfile and isfile("AutoLoad.txt") then return readfile("AutoLoad.txt") end end); if success and result then return result end; return "none" end
