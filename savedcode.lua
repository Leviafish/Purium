...
HitboxSec:Toggle({ Title = "Enable Hitbox Expander", Desc = "Expand enemy body parts", Value = false, Callback = function(v) 
    _G.HitboxStatus = v 
end})
...
HitboxSec:Slider({ Title = "Hitbox Transparency", Desc = "Adjust visibility (0-100)", Value = {Min = 0, Max = 100, Default = 70}, Callback = function(v) 
...
MoveSec:Slider({ Title = "Jump Amount", Value = {Min = 50, Max = 1500, Default = 100}, Callback = function(v) 
    _G.JpValue = v 
end})
...
_G.AntiFling = false
FlingSec:Toggle({ Title = "Anti Fling", Desc = "Prevent being flung", Value = false, Callback = function(v) 
    _G.AntiFling = v 
end})
...
_G.FlingTarget = ""
FlingSec:Input({ Title = "Target Name (For Teleport)", Desc = "Enter part of the target's name", Value = "", Callback = function(v) 
    _G.FlingTarget = v 
end})

_G.FlingPower = 50000
FlingSec:Slider({ Title = "Fling Power", Desc = "Rotation speed for maximum fling", Value = {Min = 1000, Max = 100000, Default = 50000}, Callback = function(v) 
    _G.FlingPower = v 
end})
...
RunService.Stepped:Connect(function()
    pcall(function()
        ...
        if _G.FlingActive and LocalPlayer.Character then
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    for _, part in ipairs(p.Character:GetDescendants()) do
                        if part:IsA("BasePart") then
...
_G.HitsPerPacket = 100 
CombatSec:Slider({ Title = "Direct Payload Hits", Desc = "Hits sent instantly. Maximize to eradicate targets.", Value = {Min = 1, Max = 1000, Default = 100}, Callback = function(v) 
    _G.HitsPerPacket = v 
end})
...
_G.AuraRadius = 5000
CombatSec:Slider({ Title = "Kill Radius", Desc = "How far you can kill.", Value = {Min = 100, Max = 10000, Default = 5000}, Callback = function(v) 
    _G.AuraRadius = v 
end})

_G.KillAll = false
CombatSec:Toggle({ Title = " Auto Kill All Players", Desc = "I DIDNT RECOMMEND YOU USE 1000 CUZ IT BROKE YOUR DEVICE", Value = false, Callback = function(v) 
    _G.KillAll = v 
end})
...
RunService.Heartbeat:Connect(function()
    ...
    if not MyTool then 
        local packTool = LocalPlayer.Backpack:FindFirstChildOfClass("Tool")
        if packTool and Char:FindFirstChild("Humanoid") then
    ...
    local wDef = { attackCycle = { ["1"] = {knockbackMul=0, slowMult=slowMul, attackTime=0, lungeMul=0, slowTime=slowTim, hitboxSizeAdd = Vector3.new(9e9, 9e9, 9e9)} }, attackOrder = {"1"} }
    local cData = {knockbackMul=0, slowMult=slowMul, attackTime=0, lungeMul=0, slowTime=slowTim}
        ...
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if hum and hum.Health > 0 then
                ...
                if dist <= _G.AuraRadius then 
                    ...
                    local hitData = { knockback = 0, isClosestEnemy = true, origin = myPos, enemyModel = p.Character, distance = dist, direction = dir }
                    
                    for i = 1, _G.HitsPerPacket do
                        table.insert(payload, hitData)
                        hitCount = hitCount + 1
                        
                        if hitCount >= 150 then
                            local args = { "AttemptWeaponHit", { attackCycleData = cData, knockback = 0, shouldLock = true, shouldLunge = false, hitboxOffset = Vector3.new(0, 0, 0), isCritical = true, shouldSlow = isSlow, attackCooldown = 0, damage = 9e9, lungeKnockback = 0, cycleIndex = 1, slowMult = slowMul, hitboxSize = Vector3.new(9e9, 9e9, 9e9), weaponDefinition = wDef, tool = MyTool, slowTime = slowTim }, payload }
...
_G.AutoHit = false
_G.HitRange = 15
CombatSec:Toggle({ Title = "Auto Hit By Distance", Desc = "Automatically attack enemies in your range", Value = false, Callback = function(v) 
    _G.AutoHit = v 
end})
...
RunService.Heartbeat:Connect(function()
    if not _G.AutoHit or _G.KillAll then return end
    ...
    pcall(function()
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then
            ...
            if dist <= _G.HitRange then 
                ...
                local payload = {}
                for i = 1, _G.HitsPerPacket do
                    table.insert(payload, { knockback = 0, isClosestEnemy = true, origin = myPos, enemyModel = target, distance = dist, direction = dir })
                end
...
RunService.Heartbeat:Connect(function()
    if not _G.AutoFarm or _G.KillAll then return end
